...
// 获取模型名称
std::string GeminiProvider::getModelName() const { return "gemini-3.6-flash"; }
...
std::string GeminiProvider::sendMessage(
    ...
    // 发送POST请求
    auto response =
        client.Post("/v1beta/openai/chat/completions", headers, requestBodyStr, "application/json");
    if (!response) {
        ERR("GeminiProvider::sendMessage: failed to send request, error: {}",
...
std::string GeminiProvider::sendMessageStream(
    ...
    request.response_handler = [&](const httplib::Response& res) {
        ...
        if (statusCode != 200) {
            ...
            ERR("GeminiProvider HTTP error: {}", errorMsg);
            return false;
        }
        ...
        while ((pos = buffer.find("\n\n")) != std::string::npos) {
            ...
            if (line.compare(0, 6, "data: ") == 0) {
                ...
                if (Json::parseFromStream(reader, ss, &chunk, &errors)) {
                    ...
                    ERR("GeminiProvider::sendMessageStream: failed to parse json chunk, errors: {}",
                        errors);
                    return false;
                }
            }
    ...
    if (!res) {
        ERR("GeminiProvider::sendMessageStream: failed to send request, error: {}",
            to_string(res.error()));
        return "";
    }
    ...
    // 确保流式操作正式结束
    if (!streamFinish) {
        WRN("stream ended without [DONE] marker");
        callback("", true);
    }
